...
/**
 * DefaultMQProducerImpl 实现了 MQProducerInner 接口，会向 MQClientInstance 注册，接受管理。当首次启动时，会启动 MQClientInstance
 */
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 改变生产者的 instanceName 为进程 ID，如果 instanceName 为默认值 DEFAULT 的话，RocketMQ 会自动将 instanceName 设置为进程 ID
                ...
                // 向 MQClientInstance 注册，将当前生产者加入到 MQClientInstance 管理中（ConcurrentMap<String/* group */, MQProducerInner> producerTable），方便后续调用网络请求、进行心跳检测等
     ...
     * @param isolation      isolation，是否隔离，该参数的含义如果为 true，则使用默认时长 30s 来计算 Broker 故障规避时长，如果为 false，则使用本次消息发送延迟时间来计算 Broker 故障规避时长
     */
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 消息发送端采用重试机制 ，由 retryTimesWhenSendFailed 指定同步方式重试次数，异步重试机制在收到消息发送结果后执行回调之前进行重试 retryTimesWhenSendAsyncFailed 指定
            int timesTotal = communicationMode == CommunicationMode.SYNC ? 1 + this.defaultMQProducer.getRetryTimesWhenSendFailed() : 1;
            int times = 0;
            String[] brokersSent = new String[timesTotal];
            // 循环执行，选择消息队列、发送消息，发送成功则返回，收到异常则重试。选择消息队列有两种方式：1）sendLatencyFaultEnable=false，默认不启用 Broker 故障延迟机制，2）sendLatencyFaultEnable=true，启用 Broker 故障延迟机制
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // 如果发送过程中抛出了异常，则调用 DefaultMQProducerlmpl#updateFaultItem
                        endTimestamp = System.currentTimeMillis();
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 如果生产者中缓存了 topic 的路由信息，如果该路由信息中包含了消息队列，则直接返回该路由信息，如果没有缓存或没有包含消息队列，则向 nameServer 查询该 topic 的路由信息。
        // 如果最终未找到路由信息，则抛出异常 无法找到主题相关路由信息异常
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // 如果路由信息未找到，再次尝试用默认主题 DefaultMQProducerImpl#createTopicKey 去查询，如果 BrokerConfig#autoCreateTopicEnable 为 true 时，NameServer 将返回路由信息，
            // 如果 autoCreateTopicEnabe 为 false，将抛出无法找到 topic 路由异常。
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // 根据 MessageQueue 从 ConcurrentMap<String/* Broker Name */, HashMap<Long/* brokerId */, String/* address */>> brokerAddrTable 缓存中获取 Broker 的网络地址
        String brokerAddr = this.mQClientFactory.findBrokerAddressInPublish(mq.getBrokerName());
        // 如果 MQClientInstance 的 brokerAddrTable 未缓存该 Broker 的信息，则从 NameServer 主动更新一次 topic 的路由信息。
        // 如果路由更新后还是找不到 Broker 信息，则抛出 MQClientException 异常，提示 Broker 不存在
        if (null == brokerAddr) {
        ...
        if (brokerAddr != null) {
            // 获取到 brokerAddr；得到对应的 master 地址的 VIP 通道地址，就是端口号减 2
            ...
            try {
                ...
                // 如果消息体默认超过 4K，会对消息体采用 zip 压缩，并设置消息的系统标记为 MessageSysFlag.COMPRESSED_FLAG
                int sysFlag = 0;
                ...
                // 如果是事务 Prepared 消息，设置消息的系统标记为 MessageSysFlag.TRANSACTION_PREPARED_TYPE
                // 在消息发送之前，如果消息为 prepare 类型，则设置消息标准为 prepare 消息类型，方便消息服务器正确识别事务类型的消息
                ...
                // 消息标记（ RocketMQ 对消息中的 flag 不做任何处理供应用程序使用）
                ...
                // topic 是否以 MixAll.RETRY_GROUP_TOPIC_PREFIX 开头，如果是则从消息扩展属性中获取重试次数，并重新给 requestHeader 设置值
                ...
                switch (communicationMode) {
                    case ASYNC:
                        ...
                        if (msgBodyCompressed) {
                            //If msg body was compressed, msgbody should be reset using prevBody. 如果消息体被压缩了，应该用 prevBody 重置 msgBody
                ...
                // 如果注册了消息发送钩子函数，执行 after 逻辑。注意，就算消息发送中发生 RemotingException、MQBrokerException、InterruptedException 时该方法也会执行
                if (this.hasSendMessageHook()) {
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        switch (sendResult.getSendStatus()) {
            case SEND_OK: {
                // 如果消息发送成功，则执行 TransactionListener#executeLocalTransaction() 方法，该方法的职责是记录事务消息的本地事务状态，例如可以通过将消息唯一 ID 存储在数据中，并且该方法与事务代码处于同一个事务中
                // 与业务事务要么一起成功，要么一起失败这里是事务消息设计的关键理念之一，为后续的事务状态回查提供唯一依据
                try {
        ...
        try {
            ...
            // 由于 this.endTransaction() 的执行，其业务事务并没有提交，故在使用事务消息 TransactionListener＃execute 方法除了记录事务消息状态后，应该返回 LocalTransaction.UNKNOW，事务消息的提交与回滚通过下面提到的事务消息状态回查时
            // 再决定是否提交或回滚
    ...
    public void endTransaction(
        ...
        // 根据消息所属的消息队列获取 Broker 的 IP 与端口信息，然后发送结束事务命令。其关键就是根据本地执行事务的状态分别发送提交、回滚或 "不作为" 的命令。
        ...
        // Broker 服务端的结束事务处理器为：EndTransactionProcessor
