...
/**
 * 消费组订阅信息配置信息存储在 Broker 的 ${ROCKET_HOME}/store/config/subscriptionGroup.json，默认情况下 BrokerConfig.autoCreateSubscriptionGroup 默认为 true，表示在第一次使用消费组配置信息时如果不存在，则使用上述默认值自动创建一个
 * 如果为 false，则只能通过客户端命令 mqadmin updateSubGroup 创建后修改相关参数
 */
