 ...
 * |                  | 索引每个 hash 槽占4字节      |   hashcode  |  phyoffset  |   timedif |  pre index no |
 * -------------------------------------------------------------------------------------------------------
 *
 * Hash 槽：IndexFile 默认包含 500 万个 Hash 槽，每个 Hash 槽存储的是落在该 Hash 槽的 hashcode 最新的 Index 的索引，每个 hash 槽占4个字节
 ...
 * 4、prelndexNo ：该条目的前一条记录的 Index 索引，当出现 hash 冲突时，构建的链表结构。
 */
public class IndexFile {
    ...
    public boolean putKey(final String key, final long phyOffset, final long storeTimestamp) {
        // 如果当前已使用条目数大于等于允许最大条目数时，则返回 false，表示当前索引文件已写满
        if (this.indexHeader.getIndexCount() < this.indexNum) {
            ...
            try {
                ...
                // 将 hashcode、消息物理偏移量、消息存储时间戳与索引文件时间戳、当前 Hash 槽的值存入 MappedByteBuffer
                // 写入 hashcode（4个字节）
                ...
                //beginTimestamp 、更新 ndP hOffset endTimestamp 、当前文件使用索引条目等信息
        ...
        if (this.mappedFile.hold()) {
            ...
            try {
                ...
                if (slotValue <= invalidIndex || slotValue > this.indexHeader.getIndexCount()
                    ...
                    // 由于会存在 hash 冲突，根据 slotValue 定位该 hash 槽最新的一个 Item 条目，将存储的物理偏量加入到 phyOffsets 中，然后继续验证 Item 条目中存储的上 Index 下标，如果大于等于 1 并且小于最大条目数，则继续查找 否则结束查找
